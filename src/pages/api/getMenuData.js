...
export default async function fetchMenuData(req, res) {
  ...
  try {
    const parentResponse = await axios.get(
      "http://ec2-18-143-143-173.ap-southeast-1.compute.amazonaws.com:8080/api/v1/category/anonymous/get-all-parent"
    );
    ...
    const menuPromises = parentCategories.map(async (parent) => {
      const childResponse = await axios.get(
        `http://ec2-18-143-143-173.ap-southeast-1.compute.amazonaws.com:8080/api/v1/category/anonymous/get-child?categoryId=${parent.id}`
      );
